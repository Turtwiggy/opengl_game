...

  {
    ...
    view.each([&p, &registry, &OBJECTIVE_FADE](auto entity, const auto& obj, auto& colour, const auto& highlight) {
      ...
      for (const auto& coll : p.collision_stay) {
        ...
        if (e0_layer_id == GameCollisionLayer::ACTOR_UNIT && e1_layer_id == GameCollisionLayer::ACTOR_OBJECTIVE) {
          e0_objective = e1_id;
          e1_unit = e0_id;
        }
        if (e0_layer_id == GameCollisionLayer::ACTOR_OBJECTIVE && e1_layer_id == GameCollisionLayer::ACTOR_UNIT) {
          e0_objective = e0_id;
          e1_unit = e1_id;
        }
      }
